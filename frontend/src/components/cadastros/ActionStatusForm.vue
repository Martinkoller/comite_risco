  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
      <div class="p-6 border-b border-gray-200">
        <h2 class="text-xl font-semibold">{{ item ? 'Editar' : 'Novo' }} Status (Plano de Ação)</h2>
      </div>
...
const handleSubmit = async () => {
  ...
  try {
    if (props.item) {
      await api.put(`/lookups/action-statuses/${props.item.id}`, form.value)
    } else {
      await api.post('/lookups/action-statuses', form.value)
    }
    emit('saved')
  } catch (error) {
