  ...
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999]">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full mx-4">
      <div class="p-6 border-b border-gray-200">
        <h2 class="text-xl font-semibold">{{ item ? 'Editar' : 'Novo' }} Participante</h2>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        ...
        <div>
          ...
          <input v-model="form.role" type="text" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    if (props.item) {
      await api.put(`/lookups/participants/${props.item.id}`, form.value)
    } else {
      await api.post('/lookups/participants', form.value)
    }
    emit('saved')
  } catch (error) {
