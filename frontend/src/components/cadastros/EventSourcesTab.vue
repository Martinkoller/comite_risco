<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg font-medium text-gray-900">Fonte do Evento</h2>
...
import EventSourceForm from './EventSourceForm.vue'
...
const fetchItems = async () => {
  loading.value = true
  try {
    const response = await api.get('/lookups/event-sources')
...
const deleteItem = async (id) => {
  ...
  try {
    await api.delete(`/lookups/event-sources/${id}`)
    await fetchItems()
  } catch (error) {
