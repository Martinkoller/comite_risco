<template>
  <div>
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg font-medium text-gray-900">Status (Plano de Ação)</h2>
...
import ActionStatusForm from './ActionStatusForm.vue'
...
const fetchItems = async () => {
  loading.value = true
  try {
    const response = await api.get('/lookups/action-statuses')
...
const deleteItem = async (id) => {
  ...
  try {
    await api.delete(`/lookups/action-statuses/${id}`)
    await fetchItems()
  } catch (error) {
